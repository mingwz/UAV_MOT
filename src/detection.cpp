...
bool FairMOTDetector::generate_engine(std::string& onnx_model_name, std::string& engine_model_name)
{
	...
	config->setMaxWorkspaceSize(1024 * (1 << 20));  // 1G 用于存储中间缓冲区和临时数据的内存空间

	config->setFlag(nvinfer1::BuilderFlag::kFP16);  // FP16 TF32 
...
{
	auto engine_model_name = params_.model_file + ".fp16.trtmodel";
